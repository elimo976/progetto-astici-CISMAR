    ...
    <div class="flex md:flex-col sm:flex-row justify-center w-full">
      ...
      <div class="relative flex flex-col items-center">
        ...
        <div class="relative w-full">
          ...
          <div
            class="absolute inset-0 xs:block hidden rounded-full w-1/2 mx-auto"
            :style="{
              background: `linear-gradient(to bottom, ${phaseColors[0]}, ${phaseColors[1]}, ${phaseColors[2]}, ${phaseColors[3]})`,
            }"
          ></div>
          ...
          <div
            class="flex flex-col lg:px-16 md:gap-24 md:px-0 sm:flex-row justify-between w-full mx-auto xs:pt-10"
          >
            <div
              v-for="(phase, index) in phases"
              :key="index"
              :class="[
                'flex flex-col items-center mb-8 md:mx-auto md:w-1/2',
                { 'sm:mb-0': index === phases.length - 1 }, // Applica sm:mb-0 solo all'ultimo elemento
              ]"
            >
              <img
                :src="phase.image"
                :alt="$t(phase.altText)"
                class="md:w-full sm:w-1/3 xs:w-2/5 md:mt-8 sm:mb-14 aspect-square object-cover shadow-lg rounded-full border-2 border-white transition-transform duration-300 transform hover:scale-150 active:scale-150 md:hover:translate-y-10 md:active:translate-y-10 hover:z-50 sm:relative"
              />
        ...
        <div v-for="(phase, index) in phases" :key="index" class="flex items-center">
          ...
          <div
            ...
            :style="{
              ...
              width: '50px',
              marginTop: 'calc(1rem + (50px * index))', // Allineamento dinamico
            }"
            :class="{ 'sm:mb-0': index === phases.length - 1, 'mb-4': index !== phases.length - 1 }"
          ></div>
